...
double saida_o[NR_NEURON_O][CAMADAS];
double saida_s[SAIDAS];
double delta_saida[SAIDAS];
double gradiente_oculta[NR_NEURON_O][CAMADAS];
double delta_oculta[NR_NEURON_O][CAMADAS];
...
void calcular_saidas(double entradas[ENTRADAS], int indice);
void treinar_RNA();
double calcular_erro(double desejado, double saida);
void calcular_delta_saida(double desejado, int indice);
void calcular_delta_oculta(int indice);
void calcular_gradiente_oculta(int indice);
void ajustar_pesos_sinapticos(double entradas[ENTRADAS], int indice);
...
void calcular_saidas(double entradas[ENTRADAS], int indice)
{
	...
	{
		saida_o[i][indice] = 0.0;
		saida_o[i][indice] += w_e_o[0][i] * 1;  // Calcula saida para bias

		for (j = 1; j < ENTRADAS + 1; j++)
		{
			saida_o[i][indice] += w_e_o[j][i] * entradas[j - 1];
		}

		// Calcular a saida de saida_o[i]
		saida_o[i][indice] = f_sigmoid(saida_o[i][indice]);
	}
	...
	for (i = 0; i < SAIDAS; i++)
	{
		...
		{
			saida_s[i] += w_o_s[j][i] * saida_o[j - 1][indice];
		}
...
void treinar_RNA()
{
	...
	for (i = 1; i <= EPOCAS; i++)
	{
		...
		for (j = 0; j < NR_AMOSTRAS; j++)
		{
			...
			// Feedforward
			for (camada = 0; camada < CAMADAS; camada++)
			{
				calcular_saidas(entradas, camada);

				// Backward (backpropagation)
				for (l = 0; l < SAIDAS; l++)
				{
					calcular_delta_saida(cj_treinamento[j][k + l], l);
					calcular_gradiente_oculta(camada);
					calcular_delta_oculta(camada);
					ajustar_pesos_sinapticos(entradas, camada);
				}
			}
		}
	}
...
void calcular_gradiente_oculta(int indice)
{
	...
	for (i = 0; i < SAIDAS; i++)
	{
		for (j = 1; j < NR_NEURON_O + 1; j++)
		{
			gradiente_oculta[j - 1][indice] = delta_saida[i] * w_o_s[j][i];
		}
	}
}
...
void calcular_delta_oculta(int indice)
{
	...
	for (i = 0; i < NR_NEURON_O; i++)
	{
		delta_oculta[i][indice] = gradiente_oculta[i][indice] * saida_o[i][indice] * (1 - saida_o[i][indice]);
	}
}
...
void ajustar_pesos_sinapticos(double entradas[ENTRADAS], int indice)
{
	...
	for (i = 0; i < SAIDAS; i++)
	{
		...
		for (j = 1; j < NR_NEURON_O + 1; j++)
		{
			w_o_s[j][i] = w_o_s[j][i] + TX_APRENDIZADO * delta_saida[i] * saida_o[j - 1][indice];
		}
	}
	...
	for (i = 0; i < NR_NEURON_O; i++)
	{
		...
		w_e_o[0][i] = w_e_o[0][i] + TX_APRENDIZADO * delta_oculta[i][indice] * 1;
		for (j = 1; j < ENTRADAS + 1; j++)
		{
			w_e_o[j][i] = w_e_o[j][i] + TX_APRENDIZADO * delta_oculta[i][indice] * entradas[j - 1];
		}
	}
}